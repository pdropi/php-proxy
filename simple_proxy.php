...
class Proxy
{
     ...
     * @var string
     */
    public static $AUTH_KEY = 'change_this_on_both_side';
    ...
    /**
     * If alse, don't let curl follow next location. Send next location to browser to follow, instead.
     * @var bool
     */

    public static $CURLOPT_FOLLOWLOCATION = false;
    ...
    /**
     * Reverse proxy address. Eg: '192.168.0.3:8080'; To disable, do not send REVERSE_PROXY or send empty
     * @var string
     */

    public static $REVERSE_PROXY = 'HTTP_REVERSE_PROXY';

     /**
     * If you set  $REVERSE_PROXY and $CURLOPT_FOLLOWLOCATION =false; above, you can add a path to new location when redirects
     * Eg: '/app';  "/browser_visible_path/"; It will be concatenated after $_SERVER['HTTP_HOST']
     * If you put trailing slash '/' on $REVERSE_PROXY, you'll probably have to put it here too.
     * @var string
     */

    public static $REVERSE_ADD_TO_NEW_LOCATION = '';
    /**
        ...
        foreach ($headerLines as $headerLine) {
            ...
            if (strpos($headerLine, 'HTTP/') === 0) {
                // Reset the output array as there may by multiple response headers
             	$temp = [];
		//store cookies before reset
		foreach ($results as $result){
			if (strpos(strtolower($result),'set-cookie')!== false) $temp[] =  $result;
		}
		$results = $temp;
		unset($temp);
                continue;
    ...
    protected static function createRequest($targetURL)
    {
	    ...
	    if($post_body){
		if (empty($data)) curl_setopt($request, CURLOPT_POSTFIELDS, $post_body);
		else curl_setopt($request, CURLOPT_POSTFIELDS, http_build_query($data, '', '&').'&'.$post_body);
	    }elseif(!empty($data)) curl_setopt($request, CURLOPT_POSTFIELDS, $data);
        }
...
//            CURLOPT_SSL_VERIFYPEER => false,
    ...
    public static function run()
    {
	    ...
	    }elseif ($loweredHeaderName === 'location' and !static::$CURLOPT_FOLLOWLOCATION) { //send next location to browser 
		if(!empty($_SERVER[static::$REVERSE_PROXY])) 
			header("$headerName: ".str_replace(
			$_SERVER[static::$REVERSE_PROXY],$_SERVER['HTTP_HOST'].static::$REVERSE_ADD_TO_NEW_LOCATION,$headerValue));
